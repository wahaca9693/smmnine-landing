...
export default function AdminSettingsPage() {
  ...
  const save = async (event: FormEvent) => {
    ...
    try {
      const payload = Object.fromEntries(Object.entries(form).filter(([key]) => key !== "siteName" && key !== "siteDescription"));
      const res = await fetch("/api/settings", { method: "POST", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify(payload) });
  ...
  return (
    <DashboardLayout>
      <div className="space-y-5 pb-6">
        ...
        <form onSubmit={save} className="glass-card rounded-2xl border border-[var(--color-border)] p-4 sm:p-5"><div className="mb-4 flex items-center gap-2"><Settings2 size={17} className="text-[var(--color-gold)]" /><h2 className="text-base font-black text-white">إعدادات الموقع والتشغيل</h2></div>{loading ? <div className="py-8 text-center text-xs text-zinc-500">جارٍ تحميل الإعدادات...</div> : <div className="grid gap-3 md:grid-cols-2"><label className="text-xs font-bold text-zinc-300">اسم الموقع<input value={form.siteName} readOnly className="mt-1.5 w-full cursor-not-allowed rounded-xl border border-[var(--color-border)] bg-black/10 px-3 py-3 text-sm text-zinc-400 outline-none" maxLength={80} /><small className="mt-1.5 block text-[10px] font-normal text-zinc-500">يُعدّل الاسم من <Link href="/admin/theme" className="font-bold text-[var(--color-gold)] hover:underline">هوية Royal Gold</Link> فقط لمنع التغييرات المتعارضة.</small></label><label className="text-xs font-bold text-zinc-300">العملة الافتراضية<input value={form.defaultCurrency} onChange={(e) => set("defaultCurrency", e.target.value.toUpperCase())} className="mt-1.5 w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] px-3 py-3 text-sm uppercase text-white outline-none focus:border-[var(--color-gold)]" maxLength={3} required /></label><label className="text-xs font-bold text-zinc-300 md:col-span-2">وصف الموقع<textarea value={form.siteDescription} readOnly rows={2} maxLength={240} className="mt-1.5 w-full cursor-not-allowed resize-y rounded-xl border border-[var(--color-border)] bg-black/10 px-3 py-3 text-sm text-zinc-400 outline-none" /><small className="mt-1.5 block text-[10px] font-normal text-zinc-500">الوصف جزء من الهوية المرئية ويُحفظ من صفحة المظهر فقط.</small></label><label className="text-xs font-bold text-zinc-300">الحد الأدنى للكريبتو (USD)<input type="number" min="0" step="0.01" value={form.cryptoMinAmount} onChange={(e) => set("cryptoMinAmount", e.target.value)} className="mt-1.5 w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] px-3 py-3 text-sm text-white outline-none focus:border-[var(--color-gold)]" /></label><label className="text-xs font-bold text-zinc-300">الحد الأدنى لـ Asiacell (USD)<input type="number" min="0" step="0.01" value={form.asiacellMinAmount} onChange={(e) => set("asiacellMinAmount", e.target.value)} className="mt-1.5 w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] px-3 py-3 text-sm text-white outline-none focus:border-[var(--color-gold)]" /></label><label className="flex items-center justify-between gap-3 rounded-xl border border-[var(--color-border)] bg-black/10 p-3 text-xs font-bold text-zinc-300 md:col-span-2"><span><b className="block text-white">API v2 للمستخدمين</b><small className="mt-1 block text-[10px] font-normal text-zinc-500">تفعيل أو إيقاف استدعاء الخدمات من مفاتيح المستخدمين.</small></span><button type="button" onClick={() => set("apiV2Enabled", !form.apiV2Enabled)} className={`relative h-7 w-12 rounded-full transition ${form.apiV2Enabled ? "bg-[var(--color-gold)]" : "bg-zinc-700"}`} aria-label="تفعيل API v2"><span className={`absolute top-1 h-5 w-5 rounded-full bg-white transition ${form.apiV2Enabled ? "right-1" : "right-6"}`} /></button></label><label className="flex items-center justify-between gap-3 rounded-xl border border-[var(--color-border)] bg-black/10 p-3 text-xs font-bold text-zinc-300 md:col-span-2"><span><b className="block text-white">السماح بالتسجيل الجديد</b><small className="mt-1 block text-[10px] font-normal text-zinc-500">عند الإيقاف لن يتمكن الزوار من إنشاء حسابات جديدة، مع بقاء الحسابات الحالية دون تغيير.</small></span><button type="button" onClick={() => set("registrationEnabled", !form.registrationEnabled)} className={`relative h-7 w-12 rounded-full transition ${form.registrationEnabled ? "bg-[var(--color-gold)]" : "bg-zinc-700"}`} aria-label="السماح بالتسجيل"><span className={`absolute top-1 h-5 w-5 rounded-full bg-white transition ${form.registrationEnabled ? "right-1" : "right-6"}`} /></button></label></div>}{message && <div className="mt-3 rounded-xl bg-emerald-500/10 p-3 text-xs font-bold text-emerald-300">{message}</div>}{error && <div className="mt-3 rounded-xl bg-red-500/10 p-3 text-xs font-bold text-red-300">{error}</div>}<button disabled={saving || loading} className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-[var(--color-gold)] to-[var(--color-gold-deep)] py-3 text-sm font-black text-black disabled:opacity-50"><Save size={16} />{saving ? "جاري الحفظ..." : "حفظ الإعدادات"}</button></form>
